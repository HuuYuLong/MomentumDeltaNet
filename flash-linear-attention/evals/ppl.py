...
def main():
    ...
    parser.add_argument('-d', '--data', type=str, default='/data/user/yhuang496/datasets/huggingface/fla_hub/pg19')
    ...
    csv_path = args.path.split("/")[-2].split("-")[0] + "_block_perplexities.csv"
    with open(csv_path, mode="w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["block_id", "perplexity"])
        for i, ppl in enumerate(results["block_perplexities"]):
            writer.writerow([i, float(ppl)])

    print(f"\nBlock-wise perplexities saved to {csv_path}")
