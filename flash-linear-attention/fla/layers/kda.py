...
class KimiDeltaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        # new add
        # decoding_flag = use_cache and (recurrent_state is not None)
        # if decoding_flag: 
        #     s_tm1 = recurrent_state 
        if mode == 'chunk':
        ...
        # new add
        # if decoding_flag:
        #     delta_S = recurrent_state - s_tm1
        #     norm_s = delta_S.norm(p=2, dim=(-1, -2)).mean()
        #     # print(f"{self.layer_idx} layer norm(Delta_S) {norm_s=}")
        #     values = norm_s.detach().float().cpu().flatten().tolist()

        #     with open("kda_decoding_NormDeltaS.csv", "a", newline="") as f:
        #         writer = csv.writer(f)
        #         writer.writerow(
        #             [ self.layer_idx ]
        #             + values
