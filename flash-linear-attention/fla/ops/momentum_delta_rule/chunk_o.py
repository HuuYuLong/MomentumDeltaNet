...
def chunk_mode_rule_fwd_kernel_o(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        b_k = tl.load(p_k, boundary_check=(0, 1))    # [BK, BT]

        # [BT, BK] @ [BK, BT] -> [BT, BT]
        b_A += tl.dot(b_q, b_k)

    base = (bos * H + i_h) * BT
    p_gamma_q = tl.make_block_ptr(gamma_mask_q + base, (T, BT), (H*BT, 1), (i_t * BT, 0), (BT, BT), (1, 0))
...
def chunk_mode_rule_bwd_kernel_dqk(
    ...
    p_log_a_cum = tl.make_block_ptr(log_a_cum, (T,), (H,), (i_t * BT,), (BT,), (0,))
    p_bt        = tl.make_block_ptr(bt,        (T,), (H,), (i_t * BT,), (BT,), (0,))
    p_log_mcum  = tl.make_block_ptr(log_mu_cum, (T,), (H,), (i_t * BT,), (BT,), (0,))
    p_q         = tl.make_block_ptr(q, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
    p_k         = tl.make_block_ptr(k, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))

    p_gamma_last_row = tl.make_block_ptr(gamma_mask_q + last_idx * (BT * H), (BT,), (1,), (0,), (BT,), (0,))


    b_log_mcum_last = tl.load(log_mu_cum + last_idx * H )
    b_log_acum_last = tl.load(log_a_cum + last_idx * H )

    b_gamma_last_row = tl.load(p_gamma_last_row, boundary_check=(0,))                # [BT]
    b_log_acum       = tl.load(p_log_a_cum, boundary_check=(0,))                    # [BT]
    b_log_mcum       = tl.load(p_log_mcum, boundary_check=(0,))                     # [BT]
    b_bt             = tl.load(p_bt, boundary_check=(0,))                          # [BT]
    b_q              = tl.load(p_q, boundary_check=(0, 1)) * scale                   # [BT, BK]
    ...
    for i_v in range(tl.cdiv(V, BV)):
        ...
        p_s  = tl.make_block_ptr(s,  (V, K), (1, V),   (i_v * BV, i_k * BK), (BV, BK), (0, 1))
        p_m  = tl.make_block_ptr(m,  (V, K), (1, V),   (i_v * BV, i_k * BK), (BV, BK), (0, 1))
        p_ds = tl.make_block_ptr(ds, (V, K), (1, V),   (i_v * BV, i_k * BK), (BV, BK), (0, 1))
        p_dm = tl.make_block_ptr(dm, (V, K), (1, V),   (i_v * BV, i_k * BK), (BV, BK), (0, 1))
                # [BV, BK]
        ...
        b_s = tl.load(p_s, boundary_check=(0, 1))                    # [BV, BK]
        b_m = tl.load(p_m, boundary_check=(0, 1))                    # [BV, BK]
        b_ds = tl.load(p_ds, boundary_check=(0, 1))                  # [BV, BK]
        b_dm = tl.load(p_dm, boundary_check=(0, 1))                  # [BV, BK]
    ...
    tl.store(p_dy, b_dy.to(p_dy.dtype.element_ty), boundary_check=(0, 1))
    tl.store(p_dz, b_dz.to(p_dz.dtype.element_ty), boundary_check=(0, 1))
    ...
    p_dlog_a_cum  = tl.make_block_ptr(d_log_a_cum,  (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
    p_dlog_mu_cum = tl.make_block_ptr(d_log_mu_cum, (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
    p_dbt         = tl.make_block_ptr(d_bt,         (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
    p_d_decay_s   = tl.make_block_ptr(d_decay_s,    (T, K), (H*K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
    
    tl.store(p_dlog_a_cum, b_dlog_a_cum.to(p_dlog_a_cum.dtype.element_ty), boundary_check=(0, 1))
    tl.store(p_dlog_mu_cum, b_dlog_mu_cum.to(p_dlog_mu_cum.dtype.element_ty), boundary_check=(0, 1))
    tl.store(p_dbt, b_dbt.to(p_dbt.dtype.element_ty), boundary_check=(0, 1))
    tl.store(p_d_decay_s, b_d_decay_s.to(p_d_decay_s.dtype.element_ty), boundary_check=(0, 1))
    ...
    p_dAttn = tl.make_block_ptr(d_Attn, (T, BT), (H*BT, 1), (i_t * BT, 0), (BT, BT), (1, 0))
    tl.store(p_dq, (b_dq * scale).to(p_dq.dtype.element_ty), boundary_check=(0, 1))
    tl.store(p_dk, b_dk.to(p_dk.dtype.element_ty), boundary_check=(0, 1))
    tl.store(p_dAttn, b_dAttn.to(p_dAttn.dtype.element_ty), boundary_check=(0, 1))
...
def chunk_mode_rule_bwd_kernel_dbyz(
    ...
    b_d_decay_s = tl.load(p_d_decay_s, boundary_check=(0,)).to(tl.float32)                # [BT]
    b_dAttn        = tl.load(p_dAttn, boundary_check=(0, 1)).to(tl.float32)               # [BT, BT]
    b_G         = tl.zeros([BT, BT], dtype=tl.float32)

    for i_kk in range(tl.cdiv(K, BK)):
        p_qt = tl.make_block_ptr(q, (T, K), (H * K, 1), (i_t * BT, i_kk * BK), (BT, BK), (1, 0))
        p_kt = tl.make_block_ptr(k, (K, T), (1, H * K), (i_kk * BK, i_t * BT), (BK, BT), (0, 1))  # trans

        b_kt = tl.load(p_kt, boundary_check=(0, 1))          # [BT, BK]
        b_qt = tl.load(p_qt, boundary_check=(0, 1)) # [BT, BK]
    ...
    p_d_gamma_mask_q = tl.make_block_ptr(d_gamma_mask_q, (T, BT), (H*BT, 1), (i_t * BT, 0), (BT, BT), (1, 0))
    tl.store(p_d_gamma_mask_q, b_d_gamma_mask_q.to(p_d_gamma_mask_q.dtype.element_ty), boundary_check=(0, 1))
...
def chunk_bwd_kernel_dv_local(
    ...
    p_gamma_q = tl.make_block_ptr(gamma_mask_q, (BT, T), (1, H*BT), (0, i_t * BT), (BT, BT), (0, 1))
    b_gamma_mask_q = tl.load(p_gamma_q, boundary_check=(0, 1)).to(tl.float32)
    ...
    for i_v in range(tl.cdiv(V, BV)):
        ...
        b_dv = tl.dot(b_A.to(b_do.dtype), b_do)   # Attn^T @ dO
